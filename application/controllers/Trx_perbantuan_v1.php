...
class Trx_perbantuan extends CI_Controller
{
        ...
        if ($perbantuancode){
            $pekerja = $this->Trx_perbantuan_model->countperbantuan('perbantuan_karyawan_id', $this->input->post('perbantuan_kategori', TRUE),$this->input->post('perbantuan_tgllaporan', TRUE),$this->input->post('perbantuan_shift', TRUE));
             ...
             if (strtotime($this->input->post('perbantuan_mulai',TRUE)) > strtotime($this->input->post('perbantuan_selesai',TRUE))) {
                $tengah  ="24:00";
                $noll  ="00:00";
                $ptotalkerja = ((abs((strtotime($noll) - strtotime($this->input->post('perbantuan_selesai',TRUE))))/60) + ((strtotime($tengah) - strtotime($this->input->post('perbantuan_mulai',TRUE)))/60));
                $ptotal = $ptotalkerja - $this->input->post('perbantuan_istirahat',TRUE);
                }else {
                    $ptotalkerja = (strtotime($this->input->post('perbantuan_selesai',TRUE)) -  strtotime($this->input->post('perbantuan_mulai',TRUE)))/60;
                    $ptotal = $ptotalkerja - $this->input->post('perbantuan_istirahat',TRUE);
                }

                $upah = floor(($ptotal * 16714 )/60);

          
    
            $data = array(
                    'perbantuan_karyawan_id' => $this->input->post('perbantuan_karyawan_id',TRUE),
                    'perbantuan_kategori' => $perbantuan,
                    'perbantuan_master_id' => $this->input->post('perbantuan_master_id', TRUE),
                    'perbantuan_shift' => $this->input->post('perbantuan_shift', TRUE),
                    'perbantuan_mulai' => $this->input->post('perbantuan_mulai',TRUE),
                    'perbantuan_selesai' => $this->input->post('perbantuan_selesai', TRUE),
                    'perbantuan_istirahat' => $this->input->post('perbantuan_istirahat',TRUE),
                    'perbantuan_upah' => $upah,
                    'perbantuan_totalmenit' => $ptotal,
            ...
            $xdata = array(
                    ...
                    'masterdetail_mulai'=>$this->input->post('perbantuan_mulai',TRUE),
                    'masterdetail_selesai'=>$this->input->post('perbantuan_selesai',TRUE),
                    'masterdetail_totalkerja'=>$this->input->post('perbantuan_totalmenit', TRUE),
                    'masterdetail_jamkerja'=>( $ptotal /60 ),
    ...
    public function update_action() 
    {
        ...
        $ydata = array(
            ...
            'masterdetail_selesai' => $this->input->post('perbantuan_selesai',TRUE),
            'masterdetail_istirahat' => $this->input->post('perbantuan_istirahat',TRUE),
            'masterdetail_jamkerja' =>  floor($ztotal/60),
