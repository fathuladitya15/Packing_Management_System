                    ...
                    $stfgxdata=$this->Trx_stfg_model->stfgtampil_trxmasterper($_POST['jstfg_produk_id'][$i],$_POST['jstfg_tgllaporan'],$_POST['jstfg_shift'],'3','1', $usersupervisor);     
                        for($m = 0; $m<$stfgcuntmanual; $m++){
                            foreach ($stfgxdata as $l) {
								...
								for($p = 0; $p<$stfgbcount[$m]; $p++){
                                    foreach ($stfgydata as $j) {
                                        $aidmanual[$p] = $j['manual_id'];
                                        $bidmaster[$p] = $j['manual_master_id'];
                                        $cidkaryawan[$p] = $j['manual_karyawan_id'];
                                        $didbox[$p] = $j['manual_box'];
                                        $etotalmenit[$p] = $j['manual_totalmenit'];

                                        $didupah[$p] = floor(($etotalmenit[$p] / $waktupermanual[$m]) * $stfgdbiaya[$m]);
                                     
                                        $stfgzdata[$p] = $this->Trx_stfg_model->stfgtampil_trxmasterdetailmanual('masterdetail_id',$bidmaster[$p], $cidkaryawan[$p], $usersupervisor);

                                            $updatemanual[] = array(
                                                'manual_id'=>$aidmanual[$p],
                                                'manual_upah' => $didupah[$p]
                                            );

                                            $updatemanualdetail[] = array(
                                                'masterdetail_id'=>$stfgzdata[$p],
                                                'masterdetail_upah' => $didupah[$p]
                                            );                                       
                                            } 
                            ...
                            foreach ($xdata as $d) {
                                ...
                                $this->db->query("UPDATE trx_master SET master_bayarstfg = $biayaline[$x] WHERE master_id = '$masterid[$x]'");        
                                    ...
                                    $ldata=$this->Trx_stfg_model->tampil_trxline($_POST['jstfg_produk_id'][$i],$_POST['jstfg_tgllaporan'],$_POST['jstfg_shift'],$line[$x],$masterid[$x],$usersupervisor);                 
                                        for($l = 0; $l<$lcount[$x]; $l++){
                                            foreach ($ldata as $p) {
                                                $lidline[$l] = $p['line_id'];
                                                $lidmaster[$l] = $p['line_master_id'];
                                                $lidkaryawan[$l] = $p['line_karyawan_id'];
                                                $lupah[$l] = floor((($p['line_totalmenit'] / $waktuline[$x] ) * $biayaline [$x])) ;
                                                $mdata[$l] = $this->Trx_stfg_model->tampil_trxmasterdetailline ($lidmaster[$l], $lidkaryawan[$l] ,$usersupervisor);
                                                    ...
                                                    $updatetrxlinedetail[] = array(
                                                        'masterdetail_id'=>$mdata[$l],
                                                        'masterdetail_upah' => $lupah[$l]
                                                    ); 
                                                }
                            ...
                            for($z = 0; $z<$zcount; $z++){
                                foreach ($zdata as $f) {
                                    ...
                                    $ydata=$this->Trx_stfg_model->tampil_trxmesin($_POST['jstfg_produk_id'][$i],$_POST['jstfg_tgllaporan'],$_POST['jstfg_shift'],$mesin[$z], $usersupervisor);
                                        ...
                                        for($y = 0; $y<$ycount[$z]; $y++){
                                            foreach ($ydata as $e) {
                                                $yidmesin[$y] = $e['mesin_id'];
                                                $yidmaster[$y] = $e['mesin_master_id'];
                                                $yidkaryawan[$y] = $e['mesin_karyawan_id'];
                                                $yupah[$y] = floor((($e['mesin_totalmenit'] / $waktupermesin [$z] ) * $biayapermesin [$z])) ;
                                                $yxdata[$y] = $this->Trx_stfg_model->tampil_trxmasterdetailmesin ($yidmaster[$y], $yidkaryawan[$y], $usersupervisor);
                                                    ...
                                                    $updatetrxmesindetail[] = array(
                                                        'masterdetail_id'=>$yxdata[$y],
                                                        'masterdetail_upah' => $yupah[$y]
                                                    ); 
                                                }
