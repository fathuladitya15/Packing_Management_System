<div class="box box-default">
  ...
  <div class="box-body">
    <div class='row'>
      <div class='col-xs-12'>
        <form action="<?php echo site_url('trx_perbantuan/index'); ?>" method="get" name="search" action="">
          <div class="input-group input-group-sm">
            ...
            <?php 
                                ...
                                {
                                    ?>
                                    <a href="<?php echo site_url('trx_perbantuan'); ?>" class="btn btn-default">x</a>
                                    <?php
                                }
                            ?>
...
<div class="box box-default">
  <div class="box-header with-border">
    <h3 class="box-title">DAFTAR PERBANTUAN</h3>

    <div class="box-tools pull-right">
      <a href="<?php echo base_url() ."trx_perbantuan"; ?>" type="button" class="btn btn-box-tool"><i class="fa fa-refresh fa-spin"></i></a>
      <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
      <button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-remove"></i></button>
    </div>
  </div>
  ...
  <div class='box-body'>
        <?php                 
            if(!empty($add)){
                  if($add=='Y'){ ?>
        <a class='btn btn-success' href="<?php echo base_url(); ?>trx_perbantuan/create"><span class="fa fa-plus-circle">Tambah</span></a>
        <?php } } ?>

        <?php                 
          if(!empty($plus)){
            if($plus=='Y'){ ?>
                <a class='btn btn-warning' href="<?php echo base_url(); ?>trx_perbantuan/plus"><span class="fa fa-plus-circle">Manual</span></a>
            <?php } } ?>

            <?php if($report=='Y') { ?>
            <div class="pull-right">
              <form method="POST" action="<?php echo base_url() ."trx_perbantuan/download"; ?>">
                <div class="input-group input-group-sm" >
        ...
        <table id='dataadmin1' class='table table-bordered table-striped'>
			...
			<td width="40px"><?php echo ++$start ?></td>
                ...
                <td><?php echo $trx_perbantuan->karyawan_nama ?></td>
                <td><?php echo $trx_perbantuan->perbantuan_kategori ?></td>
                <td><?php echo $trx_perbantuan->perbantuan_shift ?></td>       
                <td><?php echo substr($trx_perbantuan->perbantuan_mulai,0,5); ?></td>
                <td><?php echo substr($trx_perbantuan->perbantuan_selesai,0,5); ?></td>
                <td><?php echo $trx_perbantuan->perbantuan_istirahat ?></td>
                <td><?php echo ($trx_perbantuan->perbantuan_totalmenit/60) ?></td>
                <td><?php echo $trx_perbantuan->perbantuan_upah ?></td>
                <td><?php echo $trx_perbantuan->perbantuan_tgllaporan ?></td>
          <td style="text-align: center; width: 100px;">
                  
                <a href="<?php echo base_url() ."trx_perbantuan/read/" .$trx_perbantuan->perbantuan_id;?>">
                ...
                if(!empty($update)){
                  if($update=='Y'){
                ?>  <a href="<?php echo base_url() ."trx_perbantuan/update/" .$trx_perbantuan->perbantuan_id;?>">
                ...
                if(!empty($delete)){
                  if($delete=='Y'){
                ?>
                  <a onclick='return konfirmasi()' href="<?php echo base_url() ."trx_perbantuan/delete/" .$trx_perbantuan->perbantuan_id;?>">
