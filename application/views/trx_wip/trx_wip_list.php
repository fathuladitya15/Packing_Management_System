...
<div class="box box-default">
  ...
  <div class="box-body">
    <div class='row'>
      <div class='col-xs-12'>
        <form action="<?php echo site_url('trx_wip/index'); ?>" method="get" name="search" action="">
          <div class="input-group input-group-sm">
            ...
            <?php 
                                ...
                                {
                                    ?>
                                    <a href="<?php echo site_url('trx_wip'); ?>" class="btn btn-default">x</a>
                                    <?php
                                }
                            ?>
...
<div class="box box-default">
  <div class="box-header with-border">
    <h3 class="box-title">DAFTAR WIP</h3>

    <div class="box-tools pull-right">
      <a href="<?php echo base_url() ."trx_wip"; ?>" type="button" class="btn btn-box-tool"><i class="fa fa-refresh fa-spin"></i></a>
      <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
      <button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-remove"></i></button>
    </div>
  </div>
  ...
  <div class='box-body'>
        <?php                 
            if(!empty($add)){
                  if($add=='Y'){ ?>
        <a class='btn btn-success' href="<?php echo base_url(); ?>trx_wip/create"><span class="fa fa-plus-circle">Tambah</span></a>
        <?php } } ?>

        <?php                 
          if(!empty($plus)){
            if($plus=='Y'){ ?>
                <a class='btn btn-warning' href="<?php echo base_url(); ?>trx_wip/plus"><span class="fa fa-plus-circle">Manual</span></a>
            <?php } } ?>

            <?php if($report=='Y') { ?>
            <div class="pull-right">
              <form method="POST" action="<?php echo base_url() ."trx_wip/download"; ?>">
                <div class="input-group input-group-sm" >
        ...
        <table id='dataadmin1' class='table table-bordered table-striped'>
            ...
            foreach ($trx_wip_data as $trx_wip)
            {
                ?>
                <tr>
                <td width="20px"><input type="checkbox"></td>
    			<td width="40px"><?php echo ++$start ?></td>
                <td><?php echo $trx_wip->wip_produk_id ?></td>
                <td><?php echo $trx_wip->produk_nama ?></td>              
                <td><?php echo $trx_wip->wip_mbox15 ?></td>
                <td><?php echo $trx_wip->wip_shift ?></td>  
                <td><?php echo $trx_wip->wip_mbox1 ?></td>               
                <td><?php echo $trx_wip->wip_mbox2 ?></td>
                <td><?php echo $trx_wip->wip_mbox3 ?></td>
                <td><?php echo $trx_wip->wip_mbox4 ?></td>
                <td><?php echo $trx_wip->wip_mbox5 ?></td>
                <td><?php echo $trx_wip->wip_mbox14 ?></td>
                <td><?php echo $trx_wip->wip_rijek ?></td>
                <td><?php echo $trx_wip->wip_total ?></td>
                <td><?php echo $trx_wip->wip_tgllaporan ?></td>
            <td style="text-align: center; width: 100px;">
                  
                <a href="<?php echo base_url() ."trx_wip/read/" .$trx_wip->wip_id;?>">
                ...
                if(!empty($update)){
                  if($update=='Y'){
                ?>  <a href="<?php echo base_url() ."trx_wip/update/" .$trx_wip->wip_id;?>">
                ...
                if(!empty($delete)){
                  if($delete=='Y'){
                ?>
                  <a onclick='return konfirmasi()' href="<?php echo base_url() ."trx_wip/delete/" .$trx_wip->wip_id;?>">
