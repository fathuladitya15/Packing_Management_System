...
<div class="box box-default">
  ...
  <div class="box-body">
    <div class='row'>
      <div class='col-xs-12'>
        <form action="<?php echo site_url('trx_stiker/index'); ?>" method="get" name="search" action="">
          <div class="input-group input-group-sm">
            ...
            <?php 
                                ...
                                {
                                    ?>
                                    <a href="<?php echo site_url('trx_stiker'); ?>" class="btn btn-default">x</a>
                                    <?php
                                }
                            ?>
...
<div class="box box-default">
  <div class="box-header with-border">
    ...
    <div class="box-tools pull-right">
      <a href="<?php echo base_url() ."trx_stiker"; ?>" type="button" class="btn btn-box-tool"><i class="fa fa-refresh fa-spin"></i></a>
      <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
      <button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-remove"></i></button>
    </div>
  </div>
  ...
  <div class='box-body'>
        <?php                 
            if(!empty($add)){
                  if($add=='Y'){ ?>
        <a class='btn btn-success' href="<?php echo base_url(); ?>trx_stiker/create"><span class="fa fa-plus-circle">Tambah</span></a>
        <?php } } ?>

        <?php                 
          if(!empty($plus)){
            if($plus=='Y'){ ?>
                <a class='btn btn-warning' href="<?php echo base_url(); ?>trx_stiker/plus"><span class="fa fa-plus-circle">Manual</span></a>
            <?php } } ?>

            <?php if($report=='Y') { ?>
            <div class="pull-right">
              <form method="POST" action="<?php echo base_url() ."trx_stiker/download"; ?>">
                <div class="input-group input-group-sm" >
        ...
        <th><input type="checkbox"></th>
            ...
            foreach ($trx_stiker_data as $trx_stiker)
            {
                ?>
          <tr>
          <td width="20px"><input type="checkbox"></td>
      			<td width="40px"><?php echo ++$start ?></td>
      			<td><?php echo $trx_stiker->stiker_karyawan_id ?></td>
            <td><?php echo $trx_stiker->karyawan_nama ?></td>
      			<td><?php echo $trx_stiker->produk_nama ?></td>
            <td><?php echo $trx_stiker->stiker_shift ?></td>
      			<td><?php echo $trx_stiker->stiker_kategori ?></td>      			
      			<td><?php echo $trx_stiker->stiker_jumlahstiker ?></td>
      			<td><?php echo $trx_stiker->stiker_upah ?></td>
      			<td><?php echo $trx_stiker->stiker_tgllaporan ?></td>
			
          <td style="text-align: center; width: 100px;">
                  
                <a href="<?php echo base_url() ."trx_stiker/read/" .$trx_stiker->stiker_id;?>">
                ...
                if(!empty($update)){
                  if($update=='Y'){
                ?>  <a href="<?php echo base_url() ."trx_stiker/update/" .$trx_stiker->stiker_id;?>">
                ...
                if(!empty($delete)){
                  if($delete=='Y'){
                ?>
                  <a onclick='return konfirmasi()' href="<?php echo base_url() ."trx_stiker/delete/" .$trx_stiker->stiker_id;?>">
