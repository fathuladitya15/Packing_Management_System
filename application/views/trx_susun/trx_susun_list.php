...
<div class="box box-default"> 
  ...
  <div class="box-body">
    <div class='row'>
      <div class='col-xs-12'>
        <form action="<?php echo site_url('trx_susun/index'); ?>" method="get" name="search" action="">
          <div class="input-group input-group-sm">
            ...
            <?php 
                                ...
                                {
                                    ?>
                                    <a href="<?php echo site_url('trx_susun'); ?>" class="btn btn-default">x</a>
                                    <?php
                                }
                            ?>
...
<div class="box box-default">
  <div class="box-header with-border">
    <h3 class="box-title">DAFTAR SUSUN</h3>

    <div class="box-tools pull-right">
      <a href="<?php echo base_url() ."trx_susun"; ?>" type="button" class="btn btn-box-tool"><i class="fa fa-refresh fa-spin"></i></a>
      <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
      <button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-remove"></i></button>
    </div>
  </div>
  ...
  <div class='box-body'>
        <?php                 
            if(!empty($add)){
                  if($add=='Y'){ ?>
        <a class='btn btn-success' href="<?php echo base_url(); ?>trx_susun/create"><span class="fa fa-plus-circle">Tambah</span></a>
        <?php } } ?>

        <?php                 
          if(!empty($plus)){
            if($plus=='Y'){ ?>
                <a class='btn btn-warning' href="<?php echo base_url(); ?>trx_susun/plus"><span class="fa fa-plus-circle">Manual</span></a>
            <?php } } ?>

            <?php if($report=='Y') { ?>
            <div class="pull-right">
              <form method="POST" action="<?php echo base_url() ."trx_susun/download"; ?>">
                <div class="input-group input-group-sm" >
        ...
        <table id='dataadmin1' class='table table-bordered table-striped'>
            ...
            foreach ($trx_susun_data as $trx_susun)
            {
                ...
                <td width="20px"><input type="checkbox"></td>
        			...
        			<td><?php echo $trx_susun->karyawan_nama ?></td>
              <td ><?php echo $trx_susun->produk_nama ?></td>
              <td><?php echo $trx_susun->susun_shift ?></td>
              ...
              <td><?php echo $trx_susun->susun_totalkrat ?></td>
        			<td><?php echo $trx_susun->susun_upah ?></td>
        			<td><?php echo $trx_susun->susun_tgllaporan ?></td>
			<td style="text-align: center; width: 100px;">
                  
                <a href="<?php echo base_url() ."trx_susun/read/" .$trx_susun->susun_id;?>">
                ...
                if(!empty($update)){
                  if($update=='Y'){
                ?>  <a href="<?php echo base_url() ."trx_susun/update/" .$trx_susun->susun_id;?>">
                ...
                if(!empty($delete)){
                  if($delete=='Y'){
                ?>
                  <a onclick='return konfirmasi()' href="<?php echo base_url() ."trx_susun/delete/" .$trx_susun->susun_id;?>">
