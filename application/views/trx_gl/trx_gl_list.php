...
<div class="box box-default"> 
  ...
  <div class="box-body">
    <div class='row'>
      <div class='col-xs-12'>
        <form action="<?php echo site_url('trx_gl/index'); ?>" method="get" name="search" action="">
          <div class="input-group input-group-sm">
            
            <div class="col-sm-9 col-xs-12"><input type="text" class="form-control" name="q" value="<?php echo $q; ?>" placeholder="Pencarian (ID Master / Module / Nama Produk )"></div>
            ...
            <?php 
                                ...
                                {
                                    ?>
                                    <a href="<?php echo site_url('trx_gl'); ?>" class="btn btn-default">x</a>
                                    <?php
                                }
                            ?>
...
<div class="box box-default">
  <div class="box-header with-border">
    <h3 class="box-title">GL MASTER</h3>

    <div class="box-tools pull-right">
      <a href="<?php echo base_url() ."trx_gl"; ?>" type="button" class="btn btn-box-tool"><i class="fa fa-refresh fa-spin"></i></a>
      <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
      <button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-remove"></i></button>
    </div>
  </div>
  ...
  <div class='box-body'>
            ...
            <div class="pull-right">
              <form method="POST" action="<?php echo base_url() ."trx_gl/download"; ?>">
                <div class="input-group input-group-sm" >
        ...
        <table id='dataadmin1' class='table table-bordered table-striped'>
            ...
            foreach ($trx_gl_data as $trx_master)
            {
                ?>
                <tr>
                <td width="20px"><input type="checkbox"></td>
			<td width="40px"><?php echo ++$start ?></td>
			<td><?php echo $trx_master->master_id ?></td>
			<td><?php echo $trx_master->module_nama ?></td>
			<td><?php 

				if ($trx_master->master_acuan_id =='1') {
				echo "STFG";
				}elseif ($trx_master->master_acuan_id =='2') {
				echo "STFG MANUAL";	
      }elseif ($trx_master->master_acuan_id =='3') {
        echo "WIP"; 
				}else{
			...
			<td><?php echo $trx_master->master_shift ?></td>
      		...
      		<td><?php echo $trx_master->master_line ?></td>
      		<td><?php echo $trx_master->master_nomesin ?></td>
            <td><?php echo $trx_master->master_tgllaporan ?></td>
			<td style="text-align: center; width: 100px;">
                  
                <a href="<?php echo base_url() ."Trx_gl/read/" .$trx_master->master_id;?>">
                ...
                if(!empty($update)){
                  if($update=='Y'){
                ?>  <a href="<?php echo base_url() ."Trx_gl/update/" .$trx_master->master_id;?>">
                ...
                if(!empty($delete)){
                  if($delete=='Y'){
                ?>
                  <a onclick='return konfirmasi()' href="<?php echo base_url() ."Trx_gl/delete/" .$trx_master->master_id;?>">
