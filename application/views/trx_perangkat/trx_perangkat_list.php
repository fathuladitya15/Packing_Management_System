...
<div class="box box-default">
  ...
  <div class="box-body">
    <div class='row'>
      <div class='col-xs-12'>
        <form action="<?php echo site_url('trx_perangkat/index'); ?>" method="get" name="search" action="">
          <div class="input-group input-group-sm">
            ...
            <?php 
                                ...
                                {
                                    ?>
                                    <a href="<?php echo site_url('trx_perangkat'); ?>" class="btn btn-default">x</a>
                                    <?php
                                }
                            ?>
...
<div class="box box-default">
  <div class="box-header with-border">
    ...
    <div class="box-tools pull-right">
      <a href="<?php echo base_url() ."trx_perangkat"; ?>" type="button" class="btn btn-box-tool"><i class="fa fa-refresh fa-spin"></i></a>
      <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
      <button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-remove"></i></button>
    </div>
  </div>
  ...
  <div class='box-body'>
        <?php                 
            if(!empty($add)){
                  if($add=='Y'){ ?>
        <a class='btn btn-success' href="<?php echo base_url(); ?>trx_perangkat/create"><span class="fa fa-plus-circle">Tambah</span></a>
        <?php } } ?>

        <?php                 
          if(!empty($plus)){
            if($plus=='Y'){ ?>
                <a class='btn btn-warning' href="<?php echo base_url(); ?>trx_perangkat/plus"><span class="fa fa-plus-circle">Manual</span></a>
            <?php } } ?>

            <?php if($report=='Y') { ?>
            <div class="pull-right">
              <form method="POST" action="<?php echo base_url() ."trx_perangkat/download"; ?>">
                <div class="input-group input-group-sm" >
        ...
        <th><input type="checkbox"></th>
            ...
            foreach ($trx_perangkat_data as $trx_perangkat)
            {
              ...
              <td><?php echo $trx_perangkat->karyawan_nama ?></td>
                ...
                <a href="<?php echo base_url() ."trx_perangkat/read/" .$trx_perangkat->perangkat_id;?>">
                ...
                if(!empty($update)){
                  if($update=='Y'){
                ?>  <a href="<?php echo base_url() ."trx_perangkat/update/" .$trx_perangkat->perangkat_id;?>">
                ...
                if(!empty($delete)){
                  if($delete=='Y'){
                ?>
                  <a onclick='return konfirmasi()' href="<?php echo base_url() ."trx_perangkat/delete/" .$trx_perangkat->perangkat_id;?>">
